...
class BirthdayTest(TestCase):
    ...
    def test_manager(self):
        ...
        doys = [getattr(obj, 'birthday_dayofyear_internal') for obj in TestModel.objects.get_upcoming_birthdays(30, dec31)]
        self.assertEqual(doys, [365, 1, 2])
        doys = [getattr(obj, 'birthday_dayofyear_internal') for obj in TestModel.objects.get_upcoming_birthdays(30, dec31, reverse=True)]
        self.assertEqual(doys, [2,1,365])
        doys = [getattr(obj, 'birthday_dayofyear_internal') for obj in TestModel.objects.get_upcoming_birthdays(30, dec31, order=False)]
        self.assertEqual(doys, [1,2,365])
    ...
    def test_exception(self):
        ...
        self.assertRaises(
            FieldError, BirthdayField().contribute_to_class, BrokenModel, 'another_birthday')
